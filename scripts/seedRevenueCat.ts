...
const PROJECT_NAME = "DEXrp";

const PRODUCT_IDENTIFIER = "dexrp_pro_monthly";
const PLAY_STORE_PRODUCT_IDENTIFIER = "dexrp_pro_monthly:monthly";

const PRODUCT_DISPLAY_NAME = "DEXrp Pro Monthly";
const PRODUCT_USER_FACING_TITLE = "DEXrp Pro";
const PRODUCT_DURATION = "P1M";

const APP_STORE_APP_NAME = "DEXrp for iOS";
const APP_STORE_BUNDLE_ID = "me.dexrp.app";
const PLAY_STORE_APP_NAME = "DEXrp for Android";
const PLAY_STORE_PACKAGE_NAME = "me.dexrp.app";

const ENTITLEMENT_IDENTIFIER = "DEXrp Pro";
const ENTITLEMENT_DISPLAY_NAME = "DEXrp Pro";
...
const PRODUCT_PRICES = [
  ...
  { amount_micros: 2990000, currency: "EUR" }, // €2.99
];
...
async function seedRevenueCat() {
  ...
  const findEntitlement = (items: any[]) =>
    items?.find(
      (e) => e.lookup_key === ENTITLEMENT_IDENTIFIER || (e as any).identifier === ENTITLEMENT_IDENTIFIER
    );

  const existingEntitlement = findEntitlement(existingEntitlements.items ?? []);
  ...
  if (existingEntitlement) {
    ...
    if (error) {
      if (error && typeof error === "object" && "type" in error && error["type"] === "resource_already_exists") {
        console.log("Entitlement already exists (not in list — fetching all)");
        const { data: allEntitlements, error: refetchError } = await listEntitlements({
          client,
          path: { project_id: project.id },
          query: { limit: 100 },
        });
        if (refetchError) throw new Error("Failed to re-list entitlements");
        const found = findEntitlement(allEntitlements.items ?? []);
        if (!found) throw new Error("Entitlement 'pro' exists in RevenueCat but could not be retrieved. Please check the RevenueCat dashboard.");
        console.log("Found entitlement via re-fetch:", found.id);
        entitlement = found;
      } else {
        console.error("Create entitlement error:", JSON.stringify(error, null, 2));
